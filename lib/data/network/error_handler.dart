  ...
  switch(dioError.type){
    ...
    case DioErrorType.response:
      if(dioError.response?.statusCode != null && dioError.response?.statusMessage != null ){
        return Failure(dioError.response?.statusCode ?? Constants.zero, dioError.response?.statusMessage ?? Constants.empty);
      }else{
...
extension DataSourceExtension on DataSource{
 Failure getFailure(){
    switch(this){
      ...
      case DataSource.INERNAL_SERVER_ERROR:
        return Failure(ResponseCode.INERNAL_SERVER_ERROR, ResponseMessage.INERNAL_SERVER_ERROR.tr());
...
class ResponseCode{
  ...
  static const int INERNAL_SERVER_ERROR = 500;// crash in server side
...
class ResponseMessage{
  ...
  static const String FORBIDDEN = AppStrings.forbiddenError; //api rejected request
  static const String UNAUTORISED = AppStrings.unauthorizedError;
   static const String NOT_FOUND = AppStrings.notFoundError;
  static const String INERNAL_SERVER_ERROR = AppStrings.internalServerError;// crash in server side
  ...
  static const String NO_INTERNET_CONNECTION = AppStrings.internalServerError;
